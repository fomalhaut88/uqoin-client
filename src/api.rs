...
pub fn send(wallet: &str, address: &str, coin: &str, 
    ...
    let transactions = prepare_transactions(
        &[
            (Some(get_order_by_symbol(coin)), U256::from_hex(address)),
...
pub fn split(wallet: &str, coin: &str, 
    ...
    let transactions = prepare_transactions(
        &[
            (Some(get_order_by_symbol(coin)), U256::from(1)),
            (fee.map(|s| get_order_by_symbol(s)), U256::from(0)),
        ],
        appdata.get_wallet_key(wallet).unwrap(),
        &coins_map
...
pub fn mining(_wallet: &str, _address: &str, _coin: &str, 
              _fee: Option<&str>, _threads: usize) -> std::io::Result<()> {
    todo!()
}
